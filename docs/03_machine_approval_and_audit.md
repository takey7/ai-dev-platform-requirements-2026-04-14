...
- **required check としての AI gate**: login-based Codex review の利用を促す運用ガイド check

### 5.2 AI gate の責務
AI gate は少なくとも以下を担保する。

- Codex review の呼び出し経路を固定する
- `OPENAI_API_KEY` を GitHub Actions に要求しない
- automatic reviews または `@codex` を使う運用を明示する

### 5.3 AI gate を required check にする条件
- check 名を固定する
- PR コメントとは分離する
- login-based な GitHub review 経路を前提にする
